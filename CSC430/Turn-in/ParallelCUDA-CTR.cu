...
uchar cpuSbox[256] = {		// forward s-box
0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30, 0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76,
0xca, 0x82, 0xc9, 0x7d, 0xfa, 0x59, 0x47, 0xf0, 0xad, 0xd4, 0xa2, 0xaf, 0x9c, 0xa4, 0x72, 0xc0,
0xb7, 0xfd, 0x93, 0x26, 0x36, 0x3f, 0xf7, 0xcc, 0x34, 0xa5, 0xe5, 0xf1, 0x71, 0xd8, 0x31, 0x15,
0x04, 0xc7, 0x23, 0xc3, 0x18, 0x96, 0x05, 0x9a, 0x07, 0x12, 0x80, 0xe2, 0xeb, 0x27, 0xb2, 0x75,
...
__device__ uchar InvSbox[256] = {	// inverse s-box
...
__device__ uchar Xtime2Sbox[256] = {
...
__device__ uchar Xtime3Sbox[256] = {
...
__device__ void InvShiftRows (uchar *state)
{
uchar tmp;
...
__device__ void MixSubColumns (uchar *state)
{
uchar tmp[4 * Nb];
...
__device__ void InvMixSubColumns (uchar *state)
{
uchar tmp[4 * Nb];
int i;
...
__device__ void AddRoundKey (unsigned *state, unsigned *key)
{
int idx;
...
#include <stdio.h>
#include <fcntl.h>
uchar* in = (uchar *) malloc(BUF_SIZE);
...
uchar* out = (uchar *) malloc(BUF_SIZE);
#ifndef unix
...
void encrypt (char *mykey, char *name, uint32_t nonce, uint64_t iv)
{
	uchar expkey[4 * Nb * (Nr + 1)];
	FILE *fd = fopen (name, "rb");
	int ch, idx = 0;
	uint32_t ctr = 1;
	uchar* ctrBlock = (uchar *) malloc(BUF_SIZE);;
	  //  The nonce and iv are read from standard input and printed to standard error.
	  //  These numbers must be used only once for a given secret key.
	memcpy(ctrBlock, (void*)&nonce, sizeof(nonce));
  	memcpy(ctrBlock + sizeof(nonce), (void*)&iv, sizeof(iv));
	...
	while( ch = getc(fd), ch != EOF ) // insert all cha into in[] arr 
	{
		...
		in[idx++] = ch;
		memcpy(ctrBlock + sizeof(nonce) + sizeof(iv), (void*)&ctr, sizeof(ctr));
    		ctr += 1;
	}
	if(idx > 0 )
	  while(idx % 16 != 0)
	  ...
	  return;
	//allocate Memory at CPU
	uchar* gpuCTR;
	uchar* gpuExpkey;
	uchar* gpuOut;
	int numSMs = getSMs();
	cudaMalloc(&gpuCTR, BUF_SIZE*sizeof(uchar)); 	cudaMalloc(&gpuOut, BUF_SIZE*sizeof(uchar));
	cudaMalloc(&gpuExpkey, 4 * Nb * (Nr + 1)*sizeof(uchar));
	//copy data from host to device
	cudaMemcpy(gpuCTR, ctrBlock,  BUF_SIZE*sizeof(uchar), cudaMemcpyHostToDevice);
	cudaMemcpy(gpuExpkey, expkey,  4 * Nb * (Nr + 1)*sizeof(uchar), cudaMemcpyHostToDevice);
	// Cuda running
	gpuEncrypt<<<numSMs,256>>>(gpuCTR, gpuExpkey, gpuOut, idx);
...
extern int __cdecl _setmode (int, int);
	...
	case 'e': encrypt(argv[2], argv[3], nonce, iv); break;
