...
//  Instrumented to time the encryption or decryption operation.
//  k. weber 07-apr-2014.

//  Uses CTR mode.
//  k. weber 15-apr-2014.

// Compile as follows:  gcc instrumented-aes-ctr.c -lrt -o instrumented-aes-ctr
...
//  Returns a time stamp in nanoseconds.
/*unsigned long int monotonicTime(void)
{
  struct timespec now;
...
uchar ctrBlock[16];
  ...
  memcpy((void*)ctrBlock, (void*)&nonce, sizeof(nonce));
  memcpy((void*)ctrBlock + sizeof(nonce), (void*)&iv, sizeof(iv));
  ...
  while( ch = getc(fd), ch != EOF ) {
    ...
    if( idx % 16 )
      continue;
    memcpy((void*)ctrBlock + sizeof(nonce) + sizeof(iv), (void*)&ctr, sizeof(ctr));
    ctr += 1;
    Encrypt (ctrBlock, expkey, out); //  Encrypt the ctr block.

    //  now xor encrypted ctr block with plaintext.
    for( idx = 0; idx < 16; idx++ )
      putchar (in[idx] ^ out[idx]);
    idx = 0;
  }
...
uint64_t iv;
  ...
  case 'e':
  case 'd': encrypt(argv[2], argv[3], nonce, iv); break;  //  encrypt and decrypt are the same with CTR mode.
  ...
  //  Print time on stderr so it doesn't get mingled with result.
  //fprintf(stderr, "Time to perform operation = %ld ns\n", cpuTime);
}
